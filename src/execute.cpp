                                ...
                                const PrivilegeLevel priv)
{
    ...
    case DIV: {
        ...
        else if (static_cast<int64_t>(reg_file[instr.rs1]) == INT64_MIN &&
                 static_cast<int64_t>(reg_file[instr.rs2]) == -1)
        {
            val = INT64_MIN;
        }
        else {
    ...
    std::println(std::cerr, "FATAL: unhandled instr_type {} at pc={:#x}", static_cast<int>(instr.op_type), pc);
    std::abort();
}
